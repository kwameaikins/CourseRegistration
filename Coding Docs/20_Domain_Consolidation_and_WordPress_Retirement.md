...
- [x] `/learn` handling for Phase 1: `KNOWSIA_APP_PUBLIC_URL` turns the rewrite into permanent redirects (`/learn` → the study host's root, `/learn/*` → same path on the study host). **Changed 2026-09-15** (founder: "replace the reg.knowsia.com/learn page with the question-bank page"): the study app's root is its question-bank landing page now, so bare `/learn` goes there, not to `/catalogue` — pre-cutover as a 307 to `/learn/question-bank` (bare `/learn` still cannot be proxied, the 2026-08-23 loop), post-cutover as the 308 to the root. Catalogue links in the nav, footer and updates page still name `/learn/catalogue` on purpose.
