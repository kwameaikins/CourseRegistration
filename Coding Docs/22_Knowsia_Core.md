...
| 2 | **Identity link** | Dec 2026 | Core identity table; identities created from each app's table; linked by exact email; both apps validate logins against Core AND their own table (dual-read) and log disagreements; RBAC roles; consent / export / delete requests. | Disagreement log silent 7 days; PIN login, the handoff and the 312 printed verify URLs unchanged; Registration's user table read-only. |
